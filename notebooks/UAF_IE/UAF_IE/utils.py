...
def table_line(cells, style=None):
    line = ""
    for cell in cells:
    ...
    if style is None:
        ...
        return '<tr style="{}">{}</tr>'.format(style, line)


# +
def display_table(title, headers, html_lines, style=""):
    header_line = ""
    for header in headers:
        header_line += "<th>{}</th>".format(header)
    header_line = "<tr>{}</tr>".format(header_line)
    display(HTML("""<h2>{}</h2><table style="{}">{} {}</table>""".format(title, style, header_line, html_lines)))
